...
import { readNumbersList, readBaseMessage, logError } from './utils/fileHandler';
...
async function engine() {
    ...
    const baseMessage = readBaseMessage();
    if (!baseMessage) {
    ...
    for (let i = 0; i < numbers.length; i++) {
        ...
        if (wasSent) {
            ...
            logError(currentNumber, 'Falha de comunicação/retorno da Evolution API. Verifique token e número.');
            failCount++;
        }
